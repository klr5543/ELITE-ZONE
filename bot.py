...
class AdvancedAI:
    ...
    async def generate_smart_response(
        ...
        # إضافة تاريخ المحادثة (التعديل 21) - محسّن!
        # زيادة الذاكرة من 8 لـ 20 رسالة لفهم أفضل للسياق
...
🧩 **التعامل مع الألغاز (مهم!):**
- لو المستخدم أعطاك لغز، حاول تحله!
- لو جاوب على لغز سابق، قيّم الجواب!
- اتذكر سياق المحادثة (من سأل اللغز؟ وش كان السؤال؟)
- لو الجواب صح، قل "صح!" أو "ممتاز!"
- لو الجواب غلط، قل "قريب!" أو "حاول مرة ثانية!"

**مثال:**
مستخدم: "لغز: ما الشي اللي يخترق الزجاج ولا يكسره؟"
رد: "همم... الضوء؟ 🤔"

نفس المستخدم: "الصوت!"
رد: {f"يا قائد! " if is_leader else ""}صح! الصوت يخترق الزجاج بدون ما يكسره! 🎯✨
