...
async def on_message(message: discord.Message):
    ...
    if results and results[0]['score'] > match_threshold:
        ...
        if not skip_result:
            ...
            ai_context = (
                "هذه بيانات من داتا ARC Raiders عن الغرض المذكور. "
                "استخدمها كسياق فقط، ولا تنسخ الجمل حرفياً، ومسموح تضيف خبرتك عن اللعبة أو تجارب اللاعبين. "
            ...
            if is_obtain_question or is_location_question:
                style_hint = (
                    "جاوب كأنك تشرح لصديقك: ابدأ بجملة مباشرة توضح أسهل طريقة يحصل فيها اللاعب على القطعة "
                    "أو أين يلقاها غالباً، ثم أضف جملة ثانية نصيحة بسيطة (مثلاً متى أفضل وقت تروح أو أي منطقة تركز عليها)."
                )
            elif is_crafting_question:
                style_hint = (
                    "اشرح باختصار ما يحتاجه اللاعب لتصنيع القطعة (عدد ونوع الموارد أو الفئة العامة لها)، "
                    "ثم أعطه نصيحة سريعة عن من وين يبدأ يجمع الموارد أو كيف يجهز نفسه."
                )
            else:
                style_hint = (
                    "اشرح الفكرة بأبسط شكل ممكن في جملتين أو ثلاث، كأنك تشرح لصديق جديد على اللعبة، "
                    "بدون تفاصيل تقنية أو تكرار ممل."
                )
            
            ai_question = (
                f"سؤال اللاعب: {question}\n\n"
                f"{style_hint}\n"
                "اكتب الإجابة بالعربي الفصيح البسيط (قريب من لهجة اللاعبين) وبدون قوائم طويلة أو نقاط."
            )
...
async def ask_ai_and_reply(message: discord.Message, question: str):
    ...
    if is_location_question or is_obtain_question:
        style_hint = (
            ...
            "استخدم النص المتوفر عن الآيتم لبناء إجابة منظمة تشبه تقسيم صفحة الويكي. "
            "الهيكل المطلوب للإجابة، ويجب الالتزام به حرفياً بالعربية:\n\n"
            "مناطق:\n"
            "1- ...\n"
            "2- ...\n"
            "3- ...\n"
            ...
            "2- ...\n"
            "3- ...\n\n"
            "املأ النقاط الثلاث بأسماء المناطق أو الزونات أو الخرائط الأكثر أهمية أولاً في قسم \"مناطق\" "
            "مثل Blue Gate (Checkpoint, Warehouse)، Dam Battlegrounds، Spaceport، Buried City وغيرها حسب ما هو موجود في النص. "
            "وفي قسم \"مصادر\" اكتب أنواع الحاويات أو الصناديق أو الأشياء التي يسقط منها الغرض "
            "لكن اكتب أسماءها بالعربي قدر الإمكان مع إمكانية وضع الاسم الإنجليزي بين قوسين، "
            "مثلاً: حافلات (Buses)، أغطية سيارات (Car Hoods)، خزائن عدة السيارات (Car Tool Cabinets)، "
            "صناديق معدنية (Metal Crates)، مولدات متنقلة (Mobile Generators)، أدراج صناعية (Industrial Drawers). "
            "يمكنك استخدام أقل من ثلاث أسطر في كل قسم إذا لم تتوفر معلومات كافية، "
            "لكن لا تضف أي سطر خارج هذا الهيكل ولا تكتب نصائح أو تحذيرات أو شروحات إضافية."
        )
    elif is_crafting_question:
        style_hint = (
            "هذا سؤال عن التصنيع. "
            "أجب بنفس أسلوب لاعب خبير مختصر. "
            "الجملة الأولى: متى ولماذا يحتاج اللاعب هذا الغرض في التصنيع أو التطوير. "
            "الجملة الثانية: فكرة سريعة عن نوع الموارد أو المراحل الأساسية للحصول عليه أو استخدامه في الورشة. "
            "الجملة الثالثة (اختيارية): نصيحة أو تحذير مختصر عن ما يجب الانتباه له أثناء التصنيع."
        )
    else:
        style_hint = (
            "اشرح الفكرة بأسلوب لاعب خبير لكن بسيط. "
            "حاول أن تكون إجابتك بين جملتين وثلاث جمل فقط، "
            "تركز فيها على أهم معلومة يحتاجها اللاعب بدون تفاصيل جانبية."
        )
    
    ai_prompt = (
        f"{style_hint}\n\n"
        f"سؤال اللاعب الأصلي: {question}\n\n"
        "اكتب الإجابة بالعربي البسيط القريب من كلام اللاعبين، "
        "والتزم تماماً بالهيكل المحدد (مناطق/مصادر مع ترقيم 1- و2- و3-)، "
        "ولا تضف أي أسطر قبل كلمة \"مناطق:\" أو بعد آخر سطر في قسم \"مصادر:\"."
    )
