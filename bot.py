...
async def ask_ai_and_reply(message: discord.Message, question: str):
    ...
    if is_location_question or is_obtain_question:
        style_hint = (
            ...
            "معك في السياق مقطع من ويكي ARC Raiders يشرح هذا الغرض وتقسيماته. "
            "المطلوب: أعد تقديم نفس محتوى صفحة الويكي تقريباً كما هو من ناحية تقسيم الأقسام وترتيبها. "
            "لا تعيد تقسيم النص إلى بنية جديدة، ولا تستخدم عناوين عامة مثل \"مناطق\" أو \"مصادر\". "
            "استخدم نفس عناوين الأقسام الموجودة في الويكي (مثل General Info, Scavenging, Main Farm Spot, Other Zones, Workshop إن وجدت) "
            "ويمكنك ترجمتها للعربية مع وضع الاسم الإنجليزي بين قوسين. "
            "تحت كل عنوان، لخص نفس الفقرات أو النقاط الموجودة في الويكي بالعربي القريب من كلام اللاعبين، "
            "بدون إضافة معلومات جديدة من خارج النص وبدون نصائح أو تحذيرات شخصية."
        )
    elif is_crafting_question:
        style_hint = (
            "هذا سؤال عن التصنيع. "
    ...
    ai_prompt = (
        ...
        "اعتمد فقط على المعلومات الموجودة في السياق (خصوصاً مقطع ويكي ARC Raiders) "
        "ولا تنقل جداول أو إحصائيات تفصيلية من الداتا إن ظهرت. "
        "نظّم إجابتك على شكل أقسام تعكس نفس تقسيم صفحة الويكي وبنفس ترتيبها، "
        "ولا تضف أقساماً جديدة أو فقرات ليست مبنية على النص الأصلي."
    )
