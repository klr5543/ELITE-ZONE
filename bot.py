...
async def ask_ai_and_reply(message: discord.Message, question: str):
    ...
    if is_location_question or is_obtain_question:
        style_hint = (
            ...
            "جاوب بأسلوب لاعب خبير مختصر من ثلاث جمل فقط:\n"
            "1) أين يُلقى غالباً حسب النص (نوع المناطق أو الحاويات المذكورة).\n"
            "2) أفضل الأماكن بالأسماء كما في الويكي (اذكر 2-3 مناطق).\n"
            "3) خطوة تنفيذية سريعة من النص (مثلاً افتح كبوت السيارات وفتّش الماكينات)، "
            "واذكر إن كان يُباع عند التجار أو لا فقط إذا النص يذكر ذلك.\n"
            "استخدم العربي القريب من كلام اللاعبين، واعتمد فقط على مقطع الويكي في السياق، "
            "وممنوع تضيف أقسام أو نقاط أو جمل إضافية خارج هذه الثلاث."
        )
    elif is_crafting_question:
        style_hint = (
            "هذا سؤال عن التصنيع. "
    ...
    ai_prompt = (
        ...
        "اكتب الإجابة بالعربي البسيط القريب من كلام اللاعبين، بثلاث جمل فقط كما هو محدد. "
        "اعتمد فقط على المعلومات الموجودة في السياق (خصوصاً مقطع ويكي ARC Raiders)، "
        "ولا تضف أي جملة أو فقرة أو قسم زائد عن الثلاث جمل المطلوبة."
    )
