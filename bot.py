...
async def ask_ai_and_reply(message: discord.Message, question: str):
    ...
    if is_location_question or is_obtain_question:
        style_hint = (
            ...
            "جاوب بأسلوب لاعب خبير يشرح لصديقه في ديسكورد، مو أسلوب رسمي. "
            "ابدأ بجملة قوية توضح أفضل مكان أو منطقتين يركز عليها اللاعب لهذا الغرض "
            "ثم وسّع الفكرة بذكر أمثلة مهمة من الخرائط أو الزونات أو أنواع الصناديق "
            "مثل Checkpoint وWarehouse وScrap Yard وغيرها إذا كانت مذكورة في النص. "
            "أضف جملة أو جملتين عن الاستراتيجية: متى تروح، إيش تحذر منه، وكيف تجهز نفسك. "
            "خلك واضح ومباشر وحماسي، لكن لا تكتب مقالة طويلة؛ من ثلاث إلى خمس جمل تكفي."
        )
    elif is_crafting_question:
        style_hint = (
            "هذا سؤال عن التصنيع. جاوب وكأنك تشرح لصديق كيف يطور تجهيزاته، "
            "اشرح بسرعة ليش هذا الغرض مهم في الترقيات أو الورشة، "
            "وعطِه فكرة عن نوع الموارد أو المراحل اللي يمر فيها عشان يجهز التصنيع. "
            "خلك مختصر وواضح ومن ثلاث إلى أربع جمل."
        )
    else:
        style_hint = (
            "اشرح الفكرة بأسلوب لاعب خبير لكن بسيط، "
            "استخدم المعلومات المرفقة من الويكي بدون نسخ حرفي، "
            "وخلك في حدود ثلاث إلى خمس جمل واضحة ومباشرة."
        )
    ...
    ai_prompt = (
        ...
        "مسموح تستخدم النقاط والفواصل بشكل طبيعي، "
        "لكن لا تستخدم قوائم مطولة أو تعداد نقطي، فقط فقرة أو فقرتين قصيرتين."
    )
