...
async def ask_ai_and_reply(message: discord.Message, question: str):
    ...
    if is_location_question or is_obtain_question:
        style_hint = (
            ...
            "استخدم المعلومات المتوفرة عن الآيتم لتكوين ملخص منظم بأربع جمل قصيرة كحد أقصى، بدون أي نصائح لعب إضافية. "
            "الجملة الأولى: وصف بسيط جداً للغرض ولماذا يُستخدم (إن وُجد مثل أنه يطور الـ Gunsmith). "
            "الجملة الثانية: أماكن الحصول عليه (أهم الخرائط أو الزونات الرئيسية) مثل Blue Gate أو Dam Battlegrounds أو غيرها. "
            "الجملة الثالثة: أنواع الحاويات أو المصادر التي يسقط منها (مثل Buses, Car Hoods, Metal Crates, Industrial Drawers) إن كانت مذكورة. "
            "الجملة الرابعة: معلومات الورشة أو إعادة التدوير/التفكيك إن وُجدت (مثل استخدامه في ترقية مستوى معين أو ما يعطيه عند إعادة التدوير). "
            "لا تضف تحذيرات أو نصائح أو كلام عن الخطورة؛ فقط معلومات مباشرة من النص عن الوصف والمصادر والورشة وإعادة التدوير."
        )
    elif is_crafting_question:
        style_hint = (
            "هذا سؤال عن التصنيع. "
    ...
    ai_prompt = (
        ...
        "والتزم بألا تتجاوز إجابتك أربع جمل قصيرة متتالية، "
        "بدون أي تعداد نقطي أو فقرة طويلة أو تمهيد قبل الدخول في صلب الجواب."
    )
